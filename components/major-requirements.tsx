...
export function MajorRequirements({ allcourses, majors, selectedMajor, setSelectedMajor }: { allcourses: string[], majors: string[], selectedMajor: string | null, setSelectedMajor: React.Dispatch<React.SetStateAction<string | null>> }) {
    const [requirements, setRequirements] = useState<MajorRequirements | null>(null)
    ...
    useEffect(() => {
        ...
        const fetchRequirements = async () => {
            setLoading(true)
            try {
                const response = await fetch(`http://localhost:8080/api/reqs?major=${encodeURIComponent(selectedMajor)}`)
                    ...
                    {req.between.length > 1 ? (
                        <div>
                            {req.between.some(option => isSubset(option.courses, allcourses)) ?
                                <>
                                    <p className="font-medium mb-1 text-green-800">[COMPLETED] Choose one of the following:</p>
                                    <ul className="list-disc pl-5 space-y-1">
                                        {req.between.map((option, optIndex) => (
                                            isSubset(option.courses, allcourses) ?
                                                <li key={optIndex} className="text-green-800">[COMPLETED] {option.courses.join(" and ")}</li> :
                                                <li key={optIndex} className="text-green-800">{option.courses.join(" and ")}</li>
                                        ))}
                                    </ul>
                                </>
                                :
                                <>
                                    <p className="font-medium mb-1">Choose one of the following:</p>
                                    <ul className="list-disc pl-5 space-y-1">
                                        {req.between.map((option, optIndex) => (
                        ...
                        <div>
                            {isSubset(req.between[0].courses, allcourses) ?
                                <p className="text-green-800">[COMPLETED] Required: {req.between[0].courses.join(" and ")}</p> :
    ...
    return (
        <div className="space-y-4">
            ...
            {loading ? (
                            ...
                            <Accordion type="multiple" className="w-full">
                                {requirements.allreqs.map((reqGroup, index) => {
                                    ...
                                    if ("numreqs" in reqGroup) {
                                        const reqType = reqGroup.requirementType;
                                        let reqName = "Unknown"
                                        ...
                                        if (reqType == 1) {
                                            ...
                                            reqText = "Complete " + reqGroup.numreqs + " unknown courses."
                                        }
                                    ...
                                    return (
                                        <AccordionItem key={index} value={`item-${index}`}>
                                            ...
                                            <AccordionContent>{renderRequirements(reqGroup.requirements)}</AccordionContent>
                                        </AccordionItem>
                                    )
                                })}
                            </Accordion>
                ...
                <div className="text-center py-8 text-muted-foreground">No requirements found for this major.</div>
            ) : (
                <div className="text-center py-8 text-muted-foreground">Select a major to view requirements.</div>
            )}
        </div>
    )
}
